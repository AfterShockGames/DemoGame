...
public static class SuperMath
{
    ...
    {
        var a = Vector3.Angle(origin, direction);

        if (a < angle)
            return direction;

        var r = Vector3.Cross(planeNormal, origin);

        var s = Vector3.Angle(r, direction);
        var rotationAngle = (s < 90 ? 1 : -1) * angle;
        var rotation = Quaternion.AngleAxis(rotationAngle, planeNormal);
