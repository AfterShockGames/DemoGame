...
namespace DemoGame.Player
{
    ...
    {
        private readonly State[] bufferedStates = new State[20];
        ...
        [SerializeField] private readonly float interpolationBackTime = 0.4f;
        private float lastBufferedStateTime;
        [SerializeField] private readonly float updateRate = 0.33f;
        ...
        public void ReceiveState(State newState)
        {
            ...
            for (var i = 0; i < bufferedStatesCount - 1; i++)
        ...
        {
            //Loop all states
            for (var i = 0; i < bufferedStatesCount; i++)
            {
                //State time is local time - state counter * interval between states
                var stateTime = lastBufferedStateTime - i * updateRate;
                ...
                if (stateTime <= Time.time - interpolationBackTime || i == bufferedStatesCount - 1)
                {
                    ...
                    var afterState = bufferedStates[Mathf.Max(i - 1, 0)];
                    var afterStateTime = lastBufferedStateTime - (i - 1) * updateRate;
                    var beforeState = bufferedStates[i];
                    var beforeStateTime = lastBufferedStateTime - i * updateRate;
                    ;
